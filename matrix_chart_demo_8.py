...
C_AXIS        = '\033[38;5;240m'  # Grey for Axis Lines
C_LABEL       = '\033[38;5;250m'  # White/Grey for Text Labels
C_PROG_DONE   = '\033[38;5;81m'   # Cyan/Blue for Progress Done (Pip Style)
C_PROG_TODO   = '\033[38;5;236m'  # Dark Grey for Progress ToDo
C_RESET       = '\033[0m'
...
def render_frame(data_main, data_green, visible_idx, chart_w, chart_h, start_time):
    ...
    # --- Y-AXIS LABELS (Log Scale Simulation) ---
    # We define labels for specific rows to simulate --parallel N
    ...
    for r in range(chart_h - 1, -1, -1):
        ...
        # Add Y-Axis Label + Vertical Separator
        if r < len(y_labels):
            # Using specific index to match log scale roughly to height
            label_idx = min(r, len(y_labels)-1)
            line_buffer += f"{C_LABEL}{y_labels[label_idx]} {C_AXIS}│ {C_RESET}"
        else:
            line_buffer += f"     {C_AXIS}│ {C_RESET}"
    ...
    # 3. Render X-Axis (Pip-Style Progress Bar)
    # Style: [━━━━━━╸──────]
    # We want the bar to be exactly chart_w wide
    
    # Calculate filled amount based on visible_idx (current frame) vs total width
    # We use visible_idx directly since it represents columns revealed
    filled_len = visible_idx
    if filled_len > chart_w: filled_len = chart_w
    
    remaining_len = chart_w - filled_len
    
    # Construct the bar
    # ━ is the heavy line, ╸ is the head, ─ is the track
    bar_filled = f"{C_PROG_DONE}" + ("━" * max(0, filled_len - 1))
    
    # Determine Head Character
    if remaining_len > 0:
        bar_head = f"{C_PROG_DONE}╸" 
    else:
        bar_head = f"{C_PROG_DONE}━" # Full stop
        
    bar_empty  = f"{C_PROG_TODO}" + ("─" * remaining_len)
    
    # Axis corner connector (7 spaces for padding to match Y labels)
    corner = f"   0 {C_AXIS}└─" 
    
    # Combine
    axis_line = f"{corner}{bar_filled}{bar_head}{bar_empty}{C_RESET}"
    output.append(axis_line)
    
    # 4. Render X-Axis Time Labels
    # Dynamic logic: Place '0s' at start, and current time at the head
    
    elapsed = time.time() - start_time
    padding = " " * 7 
    
    # Create a blank line of spaces matching chart width
    label_chars = [" "] * chart_w
    ...
    # B. Current Time Label at cursor position
    # We position it slightly behind the cursor so it doesn't run off edge
    t_str = f"{elapsed:.1f}s"
    pos = filled_len - len(t_str)
    if pos < 2: pos = 2 # Don't overwrite 0s
...
def main():
    ...
    # Layout Config
    # Reserve space for:
    # 1 row for X-axis line
    # 1 row for Time labels
    # 7 chars width for Y-axis labels (" 128 | ")
    CHART_H = 6
    CHART_W = cols - 8 
    ...
    try:
        # Main Loop
        # We iterate 1 to CHART_W to reveal the graph
        for i in range(1, CHART_W + 1):
            ...
            # Reset Cursor (Height + Axis + Labels = H + 2)
        ...
        # Move down past the graph area before exiting
